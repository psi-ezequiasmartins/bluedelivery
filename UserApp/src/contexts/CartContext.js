...
export function CartProvider({ children }) {
  ...
  return(
    <CartContext.Provider value={{ 
      ...
      AddToBasket, RemoveFromBasket, CleanBasket, 
      setDelivery: (d) => {
        const isValid = d && (d.DELIVERY_ID || d.id || d.deliveryId);
        console.log('CartContext - setDelivery chamado:', d);
        console.log('CartContext - é válido?', isValid);
        setDelivery(isValid ? d : null);
      } 
    }}>
      { children }
