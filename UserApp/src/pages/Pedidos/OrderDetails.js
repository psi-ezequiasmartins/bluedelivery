    ...
    return (
      <ScrollView>
        <Card>
          ...
          <Text style={styles.address}>{pedido?.ENDERECO_ENTREGA}</Text>
          
          {/* Coordenadas para verificação */}
          {pedido?.LATITUDE && pedido?.LONGITUDE && (
            <View style={styles.coordsContainer}>
              <Text style={styles.coordsTitle}>📍 Coordenadas GPS:</Text>
              <Text style={styles.coordsText}>
                Lat: {parseFloat(pedido.LATITUDE).toFixed(6)} | 
                Lng: {parseFloat(pedido.LONGITUDE).toFixed(6)}
              </Text>
            </View>
          )}        
        </Card>
          ...
          renderItem={({item}) => (
            <Card>
              <ListItem>
                ...
                />
                <ListItem.Content>
                  <ListItem.Title>{item?.PRODUTO_NOME || item?.PRODUTO}</ListItem.Title>
              ...
              <Divider />
              <Text style={{fontWeight: "bold"}}>{item.QTD} x (R$ {parseFloat(item?.VR_UNITARIO).toFixed(2)} + R$ {parseFloat(item?.VR_ACRESCIMOS).toFixed(2)}) = R$ {parseFloat(item?.TOTAL).toFixed(2)}</Text>
            </Card>
          )}
        />
