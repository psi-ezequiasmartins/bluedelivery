...
import OrderCashPayment from "../../components/OrderPayments/CashPayment"; // Componente Dinheiro

export default function OrderPayment({ pedido }) {
  const [modalVisible, setModalVisible] = useState(false);
  const [paymentMethod, setPaymentMethod] = useState(null); // 'credit', 'pix' ou 'cash'
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      <View style={styles.container}>
        ...
        >
          <Text style={styles.paymentButtonText}>PAGAMENTO AO PORTADOR</Text>
        </TouchableOpacity>
        ...
        >
          <View style={styles.modalView}>
            <View style={styles.modalContent}>
              ...
              {paymentMethod === 'cash' && (
                <OrderCashPayment orderId={pedido?.PEDIDO_ID} />
              )}
            </View>
          </View>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  cashButton: {
    backgroundColor: "#28a745", // Verde para dinheiro
  },
  modalView: {
    flex: 1,
    justifyContent: 'center',
